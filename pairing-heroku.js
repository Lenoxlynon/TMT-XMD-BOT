/**
 * TMT-XMD-BOT - Heroku Pairing Code Server
...
// Baileys variables (will be loaded dynamically)
let makeWASocket, useMultiFileAuthState;

/**
 * Dynamically imports Baileys (ESM only)
 */
...
// HTML Interface for pairing
app.get('/', (req, res) => {
  res.send(`
  <!DOCTYPE html>
  <html>
  <head>
      <title>TMT-XMD-BOT - Heroku Pairing</title>
  ...
  <body>
      <div class="container">
          <h1>🔐 TMT-XMD-BOT</h1>
          <div class="subtitle">Heroku Pairing Code Generator</div>
          ...
          <div id="result"></div>
          <div class="footer">
              <span>⚠️ Code expires in 2 minutes • Heroku Deployment</span>
          </div>
      </div>
              ...
              try {
                  ...
                  if (data.success) {
                      ...
                      resultDiv.innerHTML = \`
                          ...
                          <p style="margin-top: 15px; font-size: 12px;">⚠️ Save this Session ID for your bot's config.js</p>
                      \`;
                  } else {
                      resultDiv.className = 'error';
...
app.post('/api/pair', async (req, res) => {
  ...
  try {
    if (cleanNumber.length < 9 || cleanNumber.length > 15) {
      return res.json({ success: false, error: 'Invalid phone number length (9-15 digits required)' });
    }
    ...
    const connectionHandler = async (update) => {
      ...
      if (qr && !pairingCode) {
        try {
          ...
          res.json({ success: true, pairingCode: pairingCode, sessionId: sessionId });
...
app.listen(PORT, async () => {
  await loadBaileys();
  console.log(`\n🔐 TMT-XMD-BOT Pairing Server (Heroku Ready)`);
